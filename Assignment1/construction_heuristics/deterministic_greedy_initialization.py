...
class Deterministic_Greedy_Initialization(Initialization_Procedure):
    ...
    def create_solution(self):
        ...
        safe_state = (
            solution.clone(), current_location, current_trip_position, current_trip_index_position, current_trip_value)
        # safe_state_2 = None
        ...
        while not solution.is_c3_satisfied():
            ...
            if ((current_trip_value + d_n_h) <= inst.get_C1()) and ((current_trip_value + d_n_c) <= inst.get_C1()):
                print("V1")
                print(d_n_c)
                safe_state = (solution.clone(), current_location, current_trip_position, current_trip_index_position,
                              current_trip_value)

                solution.change_from_delta(
                    Delta([], [(nearest_unserved_customer, current_trip_position, current_trip_index_position)]))
            ...
            elif ((current_trip_value + d_n_h) <= inst.get_C1()) and d_n_h > 0 and (
                    (current_trip_value + d_n_c) > inst.get_C1()):
                print("2")
                solution.change_from_delta(
                    Delta([], [(nearest_hotel, current_trip_position, current_trip_index_position)]))
